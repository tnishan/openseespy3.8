...
def build_model(numBayX, numBayY,numFloor,bayWidthX,bayWidthY,storyHeights,mat_KSP_unconf, mat_KSP_conf, mat_GMP,sections, C_L, m_1, n_integration_pts = 8):
    """
...
#                 ops.load(nodeTag, 5,0,-loadz,0,0,0)
                if k == 0:
    ...
    for k in range(0, numFloor):
        for i in range(0, numBayX+1):
            for j in range(0, numBayY+1):
                ...
                ops.element('nonlinearBeamColumn', eleTag, nodeTag1, nodeTag2, n_integration_pts, 1, trans_tags['P_delta'])
                eleTag += 1
                nodeTag1 += 1

    nodeTag1 = 1+ (numBayX+1)*(numBayY+1)
    # ADD BEAM ELEMENTS ALONG X DIRECTION
    for j in range(1, numFloor + 1):
        for i in range(0, numBayX):
            for k in range(0, numBayY+1):
                ...
                iNode = ops.nodeCoord(nodeTag1)
                jNode = ops.nodeCoord(nodeTag2)
                ops.element('nonlinearBeamColumn', eleTag, nodeTag1, nodeTag2,n_integration_pts, 2,trans_tags['linear_beam_X'])
                eleTag += 1  
                nodeTag1 += 1
    ...
    nodeTag1 = 1+ (numBayX+1)*(numBayY+1)
    # ADD BEAM ELEMENTS ALONG Y DIRECTION
    for j in range(1, numFloor +1):
        for i in range(0, numBayX+1):
            for k in range(0, numBayY):
                nodeTag2 = nodeTag1 + 1
                # print(nodeTag1,"-" ,nodeTag2, "   ")
                iNode = ops.nodeCoord(nodeTag1)
                jNode = ops.nodeCoord(nodeTag2)
                ops.element('nonlinearBeamColumn', eleTag, nodeTag1, nodeTag2,n_integration_pts, 2,trans_tags['linear_beam_Y'])
                eleTag += 1
                nodeTag1 += 1
            nodeTag1 += 1    
...
#      for e in all_the_cols];
    ...
    loaded_nodes = range((numBayX+1)*(numBayY+1)+1,no_nodes )
    print(loaded_nodes)
...
def run_gravity(steps = 10):
    ...
    opsplt.createODB("3DFrame", "Gravity", Nmodes=4)
...
# def run_pushover(numBayX, numBayY, numFloor, m_1, steps = 1000, direction = 'X'):
...
#     """    
#     phi = 10.0
#     no_nodes = numBayX*numBayY*(numFloor+1)
#     print(no_nodes)
    
#     if direction == 'X':
#         file_names = ['Pushover_Horizontal_ReactionsX.out',
#                       'Pushover_Story_DisplacementX.out']
#         loads = [[n,m_1*phi,0,0,0,0,0] for n in range(numBayX*numBayY+1, no_nodes)]
#         d_o_f = 1
#     else:
#         file_names = ['Pushover_Horizontal_ReactionsY.out',
#               'Pushover_Story_DisplacementY.out']
#         loads = [[n,0,m_1*phi,0,0,0,0] for n in range(numBayX*numBayY+1,no_nodes)]
#         d_o_f = 2
...
#                  '-time','-node', *list(range(1,numBayX*numBayY+1)), '-dof', d_o_f, 'reaction')
#     # records horizontal displacements of upper nodes from first floor.
#     ops.recorder('Node','-file',
#                   os.path.join('FGU_RC3DF_files',file_names[1]),
#                  '-time','-node', *list(range(numBayX*numBayY+1,numBayX*numBayY*(numFloor+1))), '-dof', d_o_f, 'disp')
...
#     ops.integrator('DisplacementControl',round(numBayX*numBayY*numFloor+numBayX*numBayY/2,0), d_o_f, step)
